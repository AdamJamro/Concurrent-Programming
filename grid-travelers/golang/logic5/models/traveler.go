...
type Traveler struct {
	...
	symbol        rune
	noOfSteps     int
	instanceType  int               // either wildlife, grid traveler, or an ambush
...
// MakeTravelers defines how grid travelers and ambushes are being made
func MakeTravelers(tileRequestChannels *TileRequestChannels, ambushSeizeRequestChannels *AmbushSeizeRequestChannels) ([config.NoOfTravelers]Traveler, [config.NoOfAmbushes]Traveler) {
	generators := config.MakeGenerators()
	travelers := [config.NoOfTravelers]Traveler{}
	ambushes := [config.NoOfAmbushes]Traveler{}

	initialPositions := generators[0].GenerateRandomInitialPositions()

	for i := range config.NoOfAmbushes {
		if errorMsg := ambushes[i].InitializeTraveler(
			TravelerData{
				Id:        i + config.NoOfTravelers + config.MaxWildlifeSpawn + 1, // + 1 is probably not needed
				Symbol:    '#',
				Generator: config.NewGenerator(), // ambushes don't use generator it can be whatever we want
				Position: Position{
					x: initialPositions[i+config.NoOfTravelers].X,
					y: initialPositions[i+config.NoOfTravelers].Y,
				},
				InstanceType: Ambush,
			},
			tileRequestChannels,
			...
			ambushSeizeRequestChannels.Channels[ambushes[i].pos] = &AmbushSeizeRequestChannel{
				Channel: make(chan AmbushSeizeRequestType),
			}
		}
	}
...
func (t *Traveler) InitializeTraveler(data TravelerData, requestChannels *TileRequestChannels) error {
	...
	case requestChannels.tiles[initialPosition.y][initialPosition.x].channel <- RequestType{
		...
		if response.ResponseCode != ResponseCodeSuccess {
			...
			time.Sleep(100) // for animation purposes
			t.addTrace()
		}
...
func (t *Traveler) Move(m MoveType, stepTimeout time.Duration, requestChannels *TileRequestChannels) ResponseType {
	...
	for {
		...
		case requestChannels.tiles[t.pos.y][t.pos.x].channel <- RequestType{
			...
			} else if response.ResponseCode == ResponseCodeAmbushed {
				fmt.Println("Traveler no.", t.id, "moved from", t.pos, "to", destinationPos, "at", response.ChangeTimestamp.Nanosecond(), "and has been seized")
				t.pos.y = destinationPos.y
				t.pos.x = destinationPos.x
				if t.instanceType == Wildlife {
					// custom kill in case of wildlife (for some reason)
					t.symbol = '*'
					t.addTraceAt(response.ChangeTimestamp)
				} else {
					t.KillAt(response.ChangeTimestamp)
				}
				time.Sleep(config.AmbushTimeout)
				t.MoveOutsideTheBoard(&requestChannels.tiles[t.pos.y][t.pos.x])
