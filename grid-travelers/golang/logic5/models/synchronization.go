...
const (
	...
	ResponseCodeNoPath   // wild traveler has no space to move over DEPRECATED!
	ResponseCodeAmbushed // traveler has been ambushed
)
...
const (
	...
	Wildlife
	GridTraveler
	Ambush       // empty ambush ready to seize
	AmbushSeized // ambush had seized
	Dead         // DEPRECATED
)
...
//func contains(ambushes [config.NoOfAmbushes]Position, pos Position) bool {
//	for _, ambush := range ambushes {
//		if ambush.x == pos.x && ambush.y == pos.y {
//			return true
//		}
//	}
//	return false
...
func InitTileServers(
	tileRequestChannels *TileRequestChannels,
	forcedMoveChannels *ForcedMoveChannels,
	ambushSeizeRequestChannels *AmbushSeizeRequestChannels,
	ctx context.Context) {

	for x := 0; x < config.GridWidth; x++ {
		for y := 0; y < config.GridHeight; y++ {
			go func() {
				err := TileServer(x, y, tileRequestChannels, forcedMoveChannels, ambushSeizeRequestChannels, ctx)
				if err != nil {
...
func log(x int, y int, state ServerState, args ...interface{}) {
	...
	case Dead:
		contentString = "DEAD"
	case Ambush:
		contentString = "AMBUSH"
	case AmbushSeized:
		contentString = "AMBUSH_SEIZED"
	default:
...
func handleEnteringOnAmbush(state *ServerState, request *RequestType) error {
	time.Sleep(time.Microsecond) // for animation purposes
	request.response <- ResponseType{
		ResponseCode:    ResponseCodeAmbushed,
		ChangeTimestamp: time.Now(),
	}
	state.content = AmbushSeized
	return nil
}

func sendSeizeRequest(state *ServerState, seizeRequest *AmbushSeizeRequestChannel) {
	time.Sleep(time.Microsecond) // for animation purposes
	// inform ambush to reprint its symbol
	seizeRequest.Channel <- AmbushSeizeRequestType{
		changeTimestamp: time.Now(),
	}
	state.content = Ambush
}

func TileServer(x int, y int, requestChannels *TileRequestChannels, forcedMoveChannels *ForcedMoveChannels, ambushSeizeRequestChannels *AmbushSeizeRequestChannels, ctx context.Context) error {
	state := ServerState{
		content: Empty,
		id:      -1,
	}
	...
	for {
		...
		case request := <-requestChannels.tiles[y][x].channel:
			...
			if (request.destination.y == config.GridHeight) && (request.destination.x == config.GridWidth) {
				if state.id != request.requester.id && state.content != AmbushSeized {
					log(x, y, state, "error: traveler", request.requester.id, "is trying to leave the board, but he is not on this tile!")
					return fmt.Errorf("error: traveler %d is trying to leave the board, but he is not on this tile!", request.requester.id)
				} else {
				}

				if state.content == AmbushSeized {
					seizeRequestChannel := ambushSeizeRequestChannels.Channels[Position{x, y}]
					if seizeRequestChannel != nil {
						sendSeizeRequest(&state, seizeRequestChannel)
					} else {
						log(x, y, state, "internal error: ambush seize request channel is nil, illegal state")
					}
					log(x, y, state, "requester", request.requester.id, "has left the board due to an ambush")
				} else {
					state.content = Empty
					state.id = -1
					log(x, y, state, "requester", request.requester.id, "has left the board")
				}
				// IMPORTANT: server does not send any response
				// and traveler does not wait for it
				continue
			}
			...
			// now we are sure the request is meant to enter this tile

			var err error
			if state.content == Ambush {
				err = handleEnteringOnAmbush(&state, &request)
			} else if state.content == GridTraveler || request.requester.kind == Wildlife || state.content == AmbushSeized {
				// basically sends an error back
