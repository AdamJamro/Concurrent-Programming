...
-- Processes 
   ...
   Min_Steps : constant Integer := 40 ;
   Max_Steps : constant Integer := 80 ;
...
-- Types, procedures and functions
  ...
  -- Processes
   ...
   for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      while Choosing(1 - Process.Id) = True loop
         if Turn /= Process_Id then
            Choosing(Process.Id) := False;
            while Turn /= Process_Id loop
               null;
            end loop;
            Choosing(Process.Id) := True;
         end if;
