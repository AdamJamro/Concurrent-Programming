...
-- Processes 
   ...
   Nr_Of_Processes : constant Integer := 2; -- DO NOT CHANGE THIS VALUE!

   Min_Steps : constant Integer := 80 ;
   Max_Steps : constant Integer := 90 ;

   Min_Delay : constant Duration := 0.01;
   Max_Delay : constant Duration := 0.03;
...
-- Types, procedures and functions
  ...
  -- task Printer collects and prints reports of traces and the line with the parameters
   ...
   -- Print the line with the parameters needed for display script:
      ...
      Put_Line("NO_EXTRA_LABELS;"); -- Place labels with extra info here (e.g. "MAX_TICKET=...;" for Backery). 
   end Printer;
  ...
  -- Processes
   ...
   Last: AtomicInteger := 0;

   task body Process_Task_Type is
   G : Generator;
   Process : Process_Type;
   Process_Id : AtomicInteger; -- store an atomic copy for conversion purposes 
   Time_Stamp : Duration;
   Nr_of_Steps: Integer;
   ...
   -- protocol variables   
   opponentTrun : AtomicInteger := 0;

   begin
   accept Init(Id: Integer; Seed: Integer; Symbol: Character) do
      Reset(G, Seed); 
      Process.Id := Id;
      Process_Id := AtomicInteger(Id);
   ...
   for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      Last := Process_Id;
      while Choosing(1 - Process.Id) = True and Last = Process_Id loop
         if Last /= Process_Id then
            Choosing(Process.Id) := False;
            while Last /= Process_Id loop
               null;
            end loop;
            Choosing(Process.Id) := True;
         end if;
