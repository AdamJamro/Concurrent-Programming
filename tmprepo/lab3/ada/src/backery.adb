...
procedure Backery is

-- extra labels for the display script
   type AtomicInteger is new Integer;
   type AtomicBoolean is new Boolean;
   pragma Atomic(AtomicInteger);
   pragma Atomic(AtomicBoolean);

   MAX_TICKET : AtomicInteger := 0;


-- Processes 

   Nr_Of_Processes : constant Integer :=10;
...
-- Types, procedures and functions
  ...
  -- task Printer collects and prints reports of traces and the line with the parameters
   ...
   paddedMaxIntegerLabel : String(1..20);
   maxIntegerLabel : String(1..20);
   ...
   -- Print the line with the parameters needed for display script:
      ...
      --  Put("EXTRA_LABELS;MAX_TICKET;=;"); 
      --  paddedMaxIntegerLabel := Ada.Text_IO.Put_Line( Image_StrMAX_TICKET );
      --  maxIntegerLabel := paddedMaxIntegerLabel(paddedMaxIntegerLabel'First .. paddedMaxIntegerLabel'Last);
      --  Put_Line(maxIntegerLabel & ";"); -- Place labels with extra info here (e.g. "MAX_TICKET=...;" for Backery).
      Put_Line("EXTRA_LABELS;MAX_TICKET;=;" & MAX_TICKET'Img & ";"); -- Place labels with extra info here (e.g. "MAX_TICKET=...;" for Backery). 
   end Printer;
  ...
  -- Processes
   ...
   Choosing: array (0 .. (Nr_Of_Processes - 1)) of AtomicBoolean := (others => False);
   Ticket: array (0 .. (Nr_Of_Processes - 1)) of AtomicInteger := (others => 0);  
   
   pragma Volatile(Choosing);
   pragma Volatile(Ticket);
   ...
   function Max return AtomicInteger is
      value: AtomicInteger := 0;
   begin
      for I in 0 .. (Nr_Of_Processes - 1) loop
         if Ticket(I) > value then
            value := Ticket(I);
         end if;
      end loop;
      return value;
   ...
   for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      Ticket(Process.Id) := Max + 1;
      Choosing(Process.Id) := False;
      for J in 0 .. (Nr_Of_Processes - 1) loop
         if J /= Process.Id then
            while Choosing(J) = True loop
               null;
            end loop;
            while Ticket(J) /= 0 and then (Ticket(J) <= Ticket(Process.Id)) and then
               (Ticket(J) /= Ticket(Process.Id) or else J <= Process.Id) loop
               null;
            end loop;
         end if;
      end loop;
      ...
      delay Min_Delay+(Max_Delay-Min_Delay)*Duration(Random(G));
      if MAX_TICKET < Ticket(Process.Id) then
         MAX_TICKET := Ticket(Process.Id);
      end if;
      -- CRITICAL_SECTION - end

      Change_State( EXIT_PROTOCOL ); -- starting EXIT_PROTOCOL
      Ticket(Process.Id) := 0; -- exit protocol
